...
const importData = (coin: CoinGeckoCoinData): MarketData => {
  ...
  if (translations.data) {
    ...
    localStorage.setItem(
      "externalLocaleState",
      JSON.stringify(externalLocaleReducer(localExternalLocaleState, translations)),
    );
  }
...
export const fetchCoinMetaData =
  ...
  async (dispatch: Dispatch<AssetState>): Promise<void> => {
    ...
    return CoinGecko.get(`coins/${asset}`).then(
      (result) => {
        const data = {
          ...(initialAssetState.data as AssetStateData),
          [MarketType.CRYPTO]: { [result.data.id]: { ...importData(result.data), meta_data: true } },
        };
...
export const fetchCryptoMarketData =
  (currency = Constants.DEFAULT_CURRENCY, priceChangePercentage = Constants.DEFAULT_PRICE_PERCENTAGE_CHANGES) =>
  async (dispatch: Dispatch<AssetState>): Promise<void> => {
    dispatch({ type: AssetStates.FETCHING, class: MarketType.CRYPTO });
    return CoinGecko.get(`coins/markets?vs_currency=${currency}&price_change_percentage=${priceChangePercentage}`).then(
      (result) => {
        const data: Record<string, MarketData> = result.data.reduce(
          (rec: Record<string, MarketData>, coin: CoinGeckoCoinData) => {
